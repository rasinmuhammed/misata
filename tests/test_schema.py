...
from misata.schema import Column, NoiseConfig, RealismConfig, Table, Relationship, SchemaConfig, ScenarioEvent
...
class TestColumn:
    ...
    def test_date_requires_start_end(self):
        ...
        col = Column(name="created_at", type="date", distribution_params={})
        assert "start" in col.distribution_params
        assert "end" in col.distribution_params
...
class TestTable:
    ...
    def test_table_accepts_workflow_configuration(self):
        """Tables can declare workflow presets explicitly."""
        table = Table(name="orders", row_count=100, workflow_preset="order", workflow_config={"strict": True})
        assert table.workflow_preset == "order"
        assert table.workflow_config["strict"] is True
...
class TestSchemaConfig:
    ...
    def test_schema_with_typed_noise_config(self):
        """Schema should accept typed noise configuration."""
        schema = SchemaConfig(
            name="Test",
            tables=[Table(name="users", row_count=10)],
            columns={
                "users": [
                    Column(name="id", type="int", distribution_params={"distribution": "uniform", "min": 1, "max": 10}),
                    Column(name="name", type="text", distribution_params={"text_type": "name"}),
                ]
            },
            noise_config=NoiseConfig(mode="analytics_safe", null_rate=0.1, typo_rate=0.2),
        )

        assert schema.noise_config is not None
        assert schema.noise_config.mode == "analytics_safe"

    def test_noise_config_validates_bounds(self):
        """Noise rates should be bounded between 0 and 1."""
        with pytest.raises(ValidationError):
            NoiseConfig(null_rate=1.5)

    def test_schema_with_realism_config(self):
        """Schema should accept typed realism configuration."""
        schema = SchemaConfig(
            name="Test",
            tables=[Table(name="users", row_count=10)],
            columns={
            ...
            realism=RealismConfig(
                ...
                reports=["privacy", "fidelity"],
                text_mode="realistic_catalog",
            ),
        )

        assert schema.realism is not None
        assert schema.realism.coherence == "standard"
        assert schema.realism.reports == ["privacy", "fidelity"]

    def test_realism_config_validates_positive_overrides(self):
        """Realism row count overrides must stay positive."""
        with pytest.raises(ValidationError):
            RealismConfig(row_count_overrides={"orders": 0})
