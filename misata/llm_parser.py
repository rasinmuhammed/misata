...
## THE VALUE CONTRACT (NON-NEGOTIABLE)

You know every domain's real vocabulary; the engine does NOT. This schema is
the ONE moment your knowledge enters the data:

1. EVERY low-cardinality string column, on ANY table in ANY domain, MUST be
   `"type": "categorical"` with realistic `choices` (4-12) + `probabilities`.
   Applies to every column named/suffixed status, type, category, tier, level,
   method, channel, segment, stage, priority, reason, kind, grade, plan, mode.
   A bare text status column falls back to generic labels and looks fake.
2. EVERY lookup/dimension table MUST carry complete `inline_data`.
3. OPEN-ENDED domain text columns (species, technique, equipment_name,
   vessel_name, dish_name, …) MUST get a top-level `"vocabulary"` entry with
   10-40 REAL values: `"vocabulary": {"species": ["Peregrine Falcon",
   "Harris Hawk", …]}`. The engine samples these directly. For any domain
   outside mainstream e-commerce/SaaS/finance this block is mandatory.
4. DERIVED NUMERIC COLUMNS are computed, never sampled: emit
   `"distribution_params": {"formula": "base_fare * surge_multiplier"}`.
   (total = quantity * unit_price; profit = revenue - cost; etc.)
5. NEVER emit filler ("Value A", "Item 1") or SaaS tier words outside SaaS.
...
- TEMPLATE TEXT: when the user asks for template-based descriptions, supplies
  templates with {placeholder} slots, or lists variable pools (connector names,
  error codes, teams, devices), emit them literally on that column:
  `"distribution_params": {"templates": [...], "variables": {"connector":
  ["Salesforce", "Snowflake"], "error_code": ["E-4001", "AUTH-403"]}}`.
  Templates may be a flat list, or a dict keyed by another column's values with
  `"depends_on": "category"` so each row's sentence matches its category
  (an Integration sentence never lands on a Bug ticket). Copy the user's own
  templates and pools verbatim when given; write 2-3 varied, business-realistic
  templates per category when not. The engine fills {slots} per row
  deterministically - never pre-expand combinations yourself.
...

{
  ...
  "vocabulary": {
    "open_ended_text_column": ["Real Value 1", "Real Value 2", "..."]
  },
  "outcome_curves": [],
  "rate_curves": [],
  "events": []
...
## ADVANCED ENGINE FEATURES — use them whenever the story fits

- STATE MACHINE: a status column on an event/lifecycle table (orders, trips,
  claims) is driven by transitions, not sampled. On the TABLE:
  "state_machine": {"state_column": "status", "initial_state": "Requested",
    "transitions": {"Requested": {"Accepted": 0.92, "Cancelled": 0.08},
                    "Accepted": {"Completed": 0.95, "Cancelled": 0.05}}}
- ROLLUP: parent summary columns (total_spent, lifetime_rides, order_count)
  MUST reconcile with child rows — never an independent distribution:
  {"name": "total_spent", "type": "float", "distribution_params":
   {"rollup": {"from_table": "orders", "fk": "customer_id", "agg": "sum", "column": "amount"}}}
  (agg: sum | count | mean | max | min; omit "column" for count.)
- DEPENDS_ON: any "X depends on / varies by Y" →
  {"depends_on": "tier", "mapping": {"Bronze": 4.99, "Silver": {"mean": 9.99, "std": 1}}}
  Cross-table via dot notation: "depends_on": "tenant_id.plan_type_id".
  RANKABLE lookups: fees/limits ascend with rank (Bronze < Silver < Platinum;
  Free < Pro < Enterprise) — in inline_data too.
- CLUSTER EFFECT: "some hospitals/drivers are better than others" — on the
  PARENT table: "cluster_effect": {"affects_table": "trips",
    "affects_columns": {"rider_rating": {"icc": 0.3, "sd_between": 0.4}}}
- PATTERN: code-shaped text (SKU, plate, booking ref) →
  {"pattern": "[A-Z]{3}-\\\\d{5}"}, never free text.
- CONSTRAINTS: "max 8 hours per employee per day" → on the table:
  "constraints": [{"name": "max_daily_hours", "type": "max_per_group",
    "group_by": ["employee_id", "work_date"], "column": "hours", "value": 8, "action": "cap"}]
- DURATIONS: emit the duration column with a realistic distribution; the
  engine orders timestamp chains and derives end = start + duration.
...
class LLMSchemaGenerator:
    ...
    PROVIDERS = {
        "groq": {
            ...
            # llama-3.3-70b's free-tier TPM (12k) fits SYSTEM_PROMPT + a full
            # schema; qwen3-32b's 6k cap rejects the request before it starts.
    ...
    def generate_from_story(
        ...
        """
        # A structured spec is a contract, not a story. The parser extracts
        # and locks the hard parts (tables, exact row counts, FKs, enums,
        # ranges, templates); the model is then asked a much smaller question:
        # refine the soft parts (realistic distributions, couplings like
        # "sentiment must match rating") WITHIN that contract. The refinement
        # prompt is a compact skeleton, not the full spec, so token-capped
        # providers never truncate it. If the model is unavailable or answers
        # badly, the deterministic parse stands on its own.
        ...
        if looks_like_spec(story):
            ...
            try:
                self._refine_spec_schema(schema, report, temperature)
            except Exception as exc:
                report.untranslated.append(f"LLM refinement skipped: {exc}")
...
Include reference tables with inline_data for lookup values and transactional tables for mass data. Output valid JSON."""
    ...
    def _refine_spec_schema(self, schema: SchemaConfig, report, temperature: float) -> int:
        """Ask the model to refine a deterministically parsed spec, then merge
        its answer under contract enforcement (see spec_prompt.merge_refinements).

        The model never sees the full spec text, only a compact skeleton plus
        the rules the parser could not translate, and it can only touch what
        the contract leaves open. Returns the number of columns refined.
        """
        from misata.spec_prompt import merge_refinements, spec_skeleton

        if self.client is None:
            raise RuntimeError("no LLM client configured")

        skeleton = spec_skeleton(schema, report)
        open_rules = [u for u in report.untranslated if "coupling" in u or "match" in u]
        rules_block = (
            "Open rules to translate if possible:\n- " + "\n- ".join(open_rules)
            if open_rules else "No open rules."
        )
        system = (
            "You refine a data schema that was parsed deterministically from a "
            "user's structured spec. The structure is locked and enforced by code: "
            "you cannot add, remove, or rename tables or columns, and you cannot "
            "change row counts, foreign keys, or the value pools/bounds of columns "
            "marked LOCKED. Improve only the realism of unlocked columns (types, "
            "distributions, text_type) and translate the open rules into couplings.\n\n"
            'Return ONLY JSON: {"overrides": {"<table>": {"<column>": '
            '{"type": "...", "distribution_params": {...}}}}}\n'
            "Allowed types: int, float, text, categorical, date, boolean.\n"
            "Allowed params: distribution (normal|lognormal|uniform), mean, std, mu, "
            "sigma, min, max, decimals, choices, probabilities, text_type, "
            "start, end (ISO dates), depends_on, mapping, templates, variables.\n"
            "For free-text description columns prefer templates: "
            '{"templates": {"Integration": ["Our {connector} sync fails with '
            '{error_code}."], "Bug": ["The {feature} crashes on {browser}."]}, '
            '"variables": {"connector": ["Salesforce", "Snowflake"], ...}, '
            '"depends_on": "category"} - grouped by another column\'s value so '
            "each row's sentence matches its row; a flat list of templates is "
            "also accepted.\n"
            "A coupling goes on the DEPENDENT column, e.g. sentiment matching a 1-5 "
            'rating: {"distribution_params": {"depends_on": "rating", "mapping": '
            '{"1": ["negative"], "2": ["negative"], "3": ["neutral"], '
            '"4": ["positive"], "5": ["positive"]}}}. depends_on must name a column '
            "that appears EARLIER in the same table. For LOCKED columns the only "
            "accepted override is such a coupling, and mapped values must stay "
            "inside the declared pool. Omit any column you would not change."
        )
        user = f"Schema skeleton:\n{skeleton}\n\n{rules_block}"
        raw = self._call_api(
        ...
        return merge_refinements(schema, report, overrides)
    ...
    def _parse_schema(self, schema_dict: Dict) -> SchemaConfig:
        ...
        for t in schema_dict.get("tables", []):
            ...
            # Table-level engine features the LLM may emit directly. Malformed
            # blocks are dropped with a warning rather than aborting the build.
            state_machine = t.get("state_machine")
            if state_machine is not None and not (
                isinstance(state_machine, dict)
                and state_machine.get("state_column")
                and isinstance(state_machine.get("transitions"), dict)
            ):
                warnings.warn(
                    f"Dropping malformed state_machine on table '{table_name_raw}' "
                    f"(needs state_column + transitions dict)"
                )
                state_machine = None
            cluster_effect = t.get("cluster_effect")
            if cluster_effect is not None and not (
                isinstance(cluster_effect, dict)
                and cluster_effect.get("affects_table")
                and isinstance(cluster_effect.get("affects_columns"), dict)
            ):
                warnings.warn(
                    f"Dropping malformed cluster_effect on table '{table_name_raw}' "
                    f"(needs affects_table + affects_columns dict)"
                )
                cluster_effect = None
            table_constraints = []
            for tc in (t.get("constraints") or []):
                if not isinstance(tc, dict):
                    continue
                try:
                    from misata.schema import Constraint
                    table_constraints.append(Constraint(**tc))
                except Exception as exc:
                    warnings.warn(
                        f"Skipping malformed constraint on '{table_name_raw}': {exc}"
                    )
            ...
            tables.append(Table(
                ...
                workflow_preset=t.get("workflow_preset"),
                workflow_config=t.get("workflow_config"),
            ))

        # Parse columns (only for transactional tables, reference tables use inline_data)
        # Models regularly nest a "columns" list INSIDE each table object
        # instead of the top-level {table: [cols]} map; hoist those so the
        # schema doesn't silently lose every column definition.
        columns_src = dict(schema_dict.get("columns") or {})
        for t in schema_dict.get("tables", []):
            if (
                isinstance(t, dict) and t.get("name")
                and isinstance(t.get("columns"), list)
                and all(isinstance(c, dict) for c in t["columns"])
                and not columns_src.get(t["name"])
            ):
                columns_src[t["name"]] = t["columns"]

        columns = {}
        for table_name, cols in columns_src.items():
        ...
        # A state machine pointed at an FK column speaks in labels; make sure
        # the parent lookup table actually contains EVERY state it can emit
        # (LLMs routinely leave "Cancelled" out of the statuses inline_data).
        for t in tables:
            sm = getattr(t, "state_machine", None)
            if not sm:
                continue
            sc = sm.get("state_column", "")
            rel = next(
                (r for r in relationships
                 if r.child_table == t.name and r.child_key == sc),
                None,
            )
            if rel is None:
                continue
            parent = next((p for p in tables if p.name == rel.parent_table), None)
            if parent is None or not parent.inline_data:
                continue
            first_row = parent.inline_data[0]
            label_key = next(
                (k for k, v in first_row.items()
                 if k != rel.parent_key and isinstance(v, str)),
                None,
            )
            ...
            if missing:
                next_id = max(
                    (int(r.get(rel.parent_key, 0)) for r in parent.inline_data
                     if str(r.get(rel.parent_key, "")).lstrip("-").isdigit()),
                    default=len(parent.inline_data),
                ) + 1
                for s in sorted(missing):
                    row = {rel.parent_key: next_id, label_key: s}
                    parent.inline_data.append(row)
                    next_id += 1
                parent.row_count = len(parent.inline_data)
                warnings.warn(
                    f"Extended {parent.name} with state-machine states missing "
                    f"from its inline_data: {sorted(missing)}"
                )

        # Schema-embedded mini-capsule: `"vocabulary": {"species": ["Peregrine
        # Falcon", …]}` — the LLM's world knowledge for niche domains, spent
        # once. Keys are lowercased column names; junk entries are dropped.
        raw_vocab = schema_dict.get("vocabulary") or schema_dict.get("vocabularies") or {}
        vocabularies = {}
        if isinstance(raw_vocab, dict):
            for k, v in raw_vocab.items():
                if isinstance(v, (list, tuple)):
                    ...
                    if len(vals) >= 2:
                        vocabularies[str(k).strip().lower()] = vals

        # Contract enforcement: a low-cardinality label column (status, type,
        # tier, …) on a transactional table must arrive with explicit choices,
        # a vocabulary entry, or a resolvable FK — otherwise the engine can only
        # fall back to structural labels. In a domain we don't recognize that
        # fallback is honest but generic, so surface it loudly.
        _LABEL_SUFFIXES = ("_type", "_status", "_category", "_tier", "_level",
                           "_method", "_channel", "_segment", "_stage",
                           "_priority", "_reason", "_kind", "_grade")
        _LABEL_NAMES = {"status", "type", "category", "tier", "level", "kind",
                        "stage", "priority", "severity", "segment", "channel",
                        "method", "grade", "plan", "mode"}
        _ref_table_names = {t.name for t in tables if t.is_reference}
        for _tname, _cols in columns.items():
            if _tname in _ref_table_names:
                continue
            for _col in _cols:
                _cl = _col.name.lower()
                if _cl not in _LABEL_NAMES and not _cl.endswith(_LABEL_SUFFIXES):
                    continue
                if _col.type in ("foreign_key", "boolean", "int", "float"):
                    continue
                _p = _col.distribution_params or {}
                if _p.get("choices") or _p.get("pattern") or _p.get("formula") \
                        or _p.get("depends_on") or _cl in vocabularies:
                    continue
                warnings.warn(
                    f"Column '{_tname}.{_col.name}' looks like a label column but "
                    f"arrived without choices or vocabulary — the schema contract "
                    f"requires explicit values; falling back to structural labels."
                )
        ...
        schema = SchemaConfig(
            ...
            vocabularies=vocabularies or None,
        )
