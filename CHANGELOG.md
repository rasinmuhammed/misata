...
- **`misata audit <dir>`: audit anyone's data, not just ours.** Points the
  story-audit invariant catalog at any folder of CSVs, whoever made them: a
  faker script, SDV, Mockaroo, a hand-written seed file, or Misata itself. It
  reports orders that shipped before they were placed, ages that disagree with
  birth dates, cancelled orders carrying tracking numbers, negative counts,
  percents above 100, 50% fraud rates, and (given a schema) FK orphans, child
  events predating their parents, and parent totals that fail to sum from
  their children. Exit codes gate CI: nonzero on high-severity findings, or on
  any finding with `--strict`. Measured on the classic faker users+orders seed
  script: coherence score 42/100 with six finding classes; the same schema
  generated by Misata audits clean.

## [0.8.2.1] - 2026-07-12

Robustness patch from a degenerate-schema sweep: nine deliberately awkward
schemas (zero rows, one row, an empty FK parent, min equal to max, unicode
column names, a self-referential FK, 120 columns, a five-level FK chain, and
a single-day date range) were generated and audited. Eight passed; one crashed.

### Fixed

- **A date range of a single day no longer crashes.** Declaring
  `start: 2025-06-15, end: 2025-06-15` (the natural way to say "everything on
  launch day") raised `ValueError: low >= high` in both the date and datetime
  paths. A single-date range now means "within that day", and a reversed range
  is swapped with a warning instead of crashing.

### Added

- **A degenerate-schema robustness suite** (`tests/test_robustness.py`) keeps
  all nine awkward shapes generating, and adds the capstone self-audit
  property: across seeds, a full multi-table story generated by the engine
  must pass the engine's own `story_audit`. A generator that fails its own
  coherence check has shipped a defect by definition; this test makes that
  contract permanent.
